...
class CLIPModel(nn.Module):
    def __init__(
        ...
        self.semantic_projection = ProjectionHead(embedding_dim=512,
                                            projection_dim=256,
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        if len(labels) == 0:
            ...
            if mode == 'train':
                loss_pred = self.pred_criterion_train(logits_img, labels) * self.img_loss_weight 
                loss_pred += self.pred_criterion_train(logits_text, labels) * self.text_loss_weight

            else:
                loss_pred = self.pred_criterion_val(logits_img, labels) * self.img_loss_weight
                loss_pred += self.pred_criterion_val(logits_text, labels) * self.text_loss_weight

        loss_pred = loss_pred / n_classifiers

        return loss_clip, loss_pred, clip_acc_text, clip_acc_images, logits_img, logits_text
